(ns stationaryplant.actor
  "StationaryPlantOperationsActor — the ISCO-08 8189 community stationaryplant actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one stationaryplant operation request
  ...
  The unconditional invariant: the StationaryPlantOperationsAdvisor can never
  directly commit a record the StationaryPlantOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [stationaryplant.advisor :as advisor]
            [stationaryplant.governor :as governor]
            [stationaryplant.store :as store]))

(defn build-graph
  "Build a compiled StationaryPlantOperationsActor graph. `store` implements
  `stationaryplant.store/Store`. `advisor` implements
  `stationaryplant.advisor/Advisor` (defaults to `mock-advisor`).
